...
//
//   1. BROAD PHASE  - "Are these two bodies even NEAR each other?"
//      Wrap each body in a cheap rectangle (AABB). If the rectangles don't
//      overlap, there's zero chance the bodies touch. Skip the expensive work.
//
//   2. NARROW PHASE - "OK they're near each other. Is any particle actually
//      INSIDE the other body?"
//      For each particle of body A, test if it's inside body B's shape.
//      If it is, figure out which edge of B is closest — that's the surface
//      we need to push the particle back out through.
//
//   3. RESOLVE      - "Push them apart and make them bounce."
//      Move the overlapping particle out (position fix), then apply velocity
//      changes (impulses) so they bounce/slide realistically.
//
// ============================================================================
...
// BROAD PHASE
// ============================================================================
// The broad phase exists purely for speed. Checking whether a point is inside
// a polygon is expensive. But checking whether two rectangles overlap is
// dirt cheap — just four comparisons. So we first wrap each body in the
// smallest axis-aligned rectangle that contains all its particles (an "AABB"
// — Axis-Aligned Bounding Box), and only do the expensive polygon test if
// those rectangles overlap.
// ============================================================================

/**
 * Compute the axis-aligned bounding box (AABB) for a body.
 *
 * Walk through every particle and track the min/max x and y.
 * The result is the tightest rectangle that contains all particles,
 * with sides parallel to the x and y axes.
 *
 * Think of it as shrink-wrapping the body with a cardboard box
 * that can't rotate.
 */
function computeAABB(body) {
    // Start with impossible extremes so the first particle always wins.
 ...
 * On one axis, they overlap when A's left edge is to the left of B's right,
 * AND A's right edge is to the right of B's left. Same idea for Y.
 *
 * If either axis has a gap, the rectangles can't be touching.
 */
...
// NARROW PHASE
// ============================================================================
// The broad phase told us "these two bodies MIGHT be colliding." Now we need
// to find out for real. We do two things:
//
//   1. Point-in-polygon: is this particle actually inside the other body?
//   2. Closest edge:     if yes, which edge should we push it out through?
//
// Together these give us a "contact" — all the info we need to fix the overlap.
// ============================================================================
...
// CONTACT COLLECTION
// ============================================================================
// Now we combine the two narrow-phase tools. For every particle of body A,
// we check if it's inside body B. If it is, we search all of B's edges to
// find the nearest one — that's the surface we'll push the particle out
// through. We record all the info we need into a "contact" object.
//
...
// ============================================================================
// This is the function that main.js calls each frame. It runs the full
// broad phase -> narrow phase -> resolve pipeline for every pair of bodies.
// ============================================================================

/**
 * Run the full collision pipeline between all pairs of bodies.
 *
 * For N bodies, we check every unique pair exactly once. (We don't need to
 * check A-vs-B AND B-vs-A as separate pairs — we handle both directions
 * within each pair.)
 *
 * For each pair, contacts are collected in BOTH directions: A's particles
 * that are inside B, AND B's particles that are inside A. Both can happen
 * at once when two bodies are deeply overlapping.
 */
function solveCollisions(bodies, elasticity, friction) {
    ...
    for (let i = 0; i < bodies.length; i++) {
        for (let j = i + 1; j < bodies.length; j++) {
            ...
            // ---- BROAD PHASE ----
            // Cheap rectangle overlap test. If their bounding boxes don't
            // touch, there's no way the actual shapes overlap. Skip.
            const aabbA = computeAABB(a);
            const aabbB = computeAABB(b);
            if (!aabbOverlap(aabbA, aabbB)) continue;

            // ---- NARROW PHASE ----
            // The bounding boxes overlap, so we need to check properly.
            // We collect contacts in both directions because either body's
            ...
            // ---- RESOLVE ----
            // Fix all the overlaps we found for this pair.
